...
console.log(students[2].name === copyStudents[2].name)
...
let bestStudents = deepCopyStudents.filter(el => el.scores > 99);
...
// - пробелом (join)
// - запятой (join)
let namesWithSpace = newDeepCopyStudents.map(el => el.name).join(' ');
console.log(namesWithSpace)
let namesWithComma = newDeepCopyStudents.map(el => el.name).join(', ');
